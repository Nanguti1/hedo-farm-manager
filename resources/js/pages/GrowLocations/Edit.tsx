...
export default function GrowLocationEdit({ growLocation, parentLocations }: PageProps) {
    const { data, setData, put, processing, errors } = useForm<GrowLocationFormData>({
        name: growLocation.name,
        type: growLocation.type,
        parent_id: growLocation.parent_id,
        area_size: growLocation.area_size,
        area_unit: growLocation.area_unit || 'acres',
        status: growLocation.status,
        description: growLocation.description || '',
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/grow-locations/${growLocation.id}`);
    };
    ...
    return (
        <AppLayout
            breadcrumbs={[
                ...
                { title: 'Edit', href: `/grow-locations/${growLocation.id}/edit` },
            ]}
        >
            <Head title={`Edit ${growLocation.name}`} />

            <div className="max-w-2xl mx-auto py-8">
                <div className="mb-6">
                    <h1 className="text-3xl font-bold">Edit Grow Location</h1>
                    <p className="text-muted-foreground">Update details for {growLocation.name}</p>
                </div>
                        ...
                        <div className="space-y-2">
                            ...
                            >
                                ...
                                <SelectContent>
                                    ...
                                    {parentLocations
                                        .filter((loc) => loc.id !== growLocation.id)
                                        .map((loc) => (

    ...
    return (
        <AppLayout breadcrumbs={[{ title: 'Inventory', href: '/inventory' }, { title: item.name, href: `/inventory/${item.id}` }]}>
            ...
            <div className="space-y-6">
                <div className="flex justify-between items-center"><div><h1 className="text-3xl font-bold">{item.name}</h1><p className="text-muted-foreground">{item.category.name}</p></div>
                    <div className="flex gap-2">
                        <Button variant="outline" asChild><Link href={`/inventory/${item.id}/edit`}>Edit Item</Link></Button>
                        <div className="bg-white p-6 rounded-lg shadow"><p className="text-sm text-gray-500">Current Quantity</p><p className={`text-3xl font-bold ${item.is_low_stock ? 'text-red-600' : ''}`}>{item.quantity} {item.unit}</p></div>
                    </div>
                </div>
